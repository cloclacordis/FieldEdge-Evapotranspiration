...
These are working development notes written in Russian during active development. They are intentionally raw — they include false starts, reconsidered decisions, and defects that were found and fixed along the way, not just final results.

This is not a development guide: what is documented here are the steps that were actually taken, not a prescription for how the steps should be taken. The program code is explained through its decisions and logic (both in the devlog and in comments within the code itself), but the process of writing the code is not described in the way it would be in proper development guides or other instructional materials.
