...
Это добавление написано как *update*: был проведен **рефакторинг** кода в файлах: **`daily-cycle.h`**, **`daily-cycle.c`**.

Проблема была в том, что функция **`RunDailyCycle()`** содержала функции печати - хотя мы сами формулировали требование, чтобы функции вычисления дневного цикла и функции печати были строго разделены: это важно уже при ближайшем проведении смоук-текста и вообще для создания портируемой прошивки (переносимого математического ядра). Функции печати при работе с МК будут заменены на драйверы УАПП (*UART*).

Однако эти элементы печати внутри функции вычислений оставались там не просто так - мы хотели сохранить возможности трассирования, мы хотели бы видеть места поломок (но также и трассировку *happy path*).

Теперь решено было создать для трассирования отдельную структуру хранения и отдельную функцию и все же строго разделить функции печати и вычисления. Функция трассирования будет **третьей функцией** в файле/модуле **`daily-cycle.c`**. Эта функция будет иметь два места вызова - при двух разных сценариях - внутри фцнкции печати общего результата ("счастливый путь") и внутри функции `main()` при ошибке и обращении к `PrintStatusAndReturn()`.

Короче говоря:

```md
RunDailyCycle()
    -> измерения, расчеты, trace
...
Стоит отметить, что `trace` - механизм, полезный не только для ПК-версии. Он останется частью `RunDailyCycle()` и на этапе тестирования прошивки на МК (по крайней мере, на этапе смоук-теста). На МК нужно будет поменять механизм вывода обеих функций печати, но логика останется той же.
