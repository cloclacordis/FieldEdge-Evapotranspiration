...
#define TOL_DEGREE     (0.01)    /* °,               latitude in degrees ***** * **** * ** */
 ...
 *
 * TC1 (isothermal): single measurement T = 20°C;
 * TC5 (min/max): three measurements - 20, 10, 30°C;
...
#define TEST_E_TMEAN_EXPECTED     (2.3383)  /* kPa,     e°(Tmean = 20°C) *** * * ***** *** */
#define TEST_E_S_EXPECTED         (2.3383)  /* kPa,     es for isothermal scenario *** *** */
#define TEST_DELTA_EXPECTED       (0.1447)  /* kPa/°C,  Δ at T = 20°C ***** * * ***** * ** */
 ...
 *     Tmax = 25.1°C; Tmin = 19.1°C, ea = 2.1 kPa;
...
/* * ** * * *** ** * * ** ****** * * * ** *** ** * * * * * * ******* * * *** * ** * **** ***
 * TC33-TC40: air humidity, atmospheric pressure, psychrometrics, vapour pressure
 *
 * TC33: AirHumidity_Update            - accumulation of min/max/mean (from ex. 5 values)
 * TC34: Calc_PressureFromElevation    - sea level
 * TC35: Calc_PressureFromElevation    - FAO56 ex. 2 (z = 1800 m)
 * TC36: Calc_AtmosphericParameters    - sea level
...
/* TC33: humidity accumulation (values from FAO56 ex. 5) ** ** * * * *** ** **** ****** ** */
...
/* * ** * * *** ** * * ** ****** * * * ** *** ** * * * * * * ******* * * *** * ** * **** ***
 * TC41-TC46: wind speed, conversion to u2 (FAO56 eq. 47)
 *
 * TC41: Calc_WindSpeedAt2m - normal path, uz = 3.2 m/s, z = 10 m -> u2 = 2.393 m/s
 * TC42: Calc_WindSpeedAt2m - z = 2 m, conversion factor ≈ 1.0
 * TC43: WindSpeed_Init + Update * 3 -> min/max/mean, then Calc_WindSpeedAt2m from mean
 * TC44: NULL pointer - WindSpeed_Init, WindSpeed_Update, Calc_WindSpeedAt2m
 * TC45: STATUS_INVALID_VALUE - speed < 0, NaN, INFINITY; height outside [0.1, 200] m
 * TC46: STATUS_INVALID_VALUE - height mismatch on subsequent Update
 * ** * * *** ** * * ** ****** * * * ** *** ** * * * * * * ******* * * *** * ** * **** *** */
...
#define TEST_WIND_SAMPLE_MS_LOW       (2.0)     /* m/s,  minimum sample TC43 **** * **** * */
#define TEST_WIND_SAMPLE_MS_MID       (3.0)     /* m/s,  middle  sample TC43 *** * *** * * */
#define TEST_WIND_SAMPLE_MS_HIGH      (4.0)     /* m/s,  maximum sample TC43 ***** * ***** */
...
#define TEST_WIND_HEIGHT_SECOND_M     (2.0)     /* m,    different height for TC46 *** *** */
#define TOL_WIND_MS                   (0.005)   /* m/s,  tolerance ***** * * * ** ****** * */

/* * ** * * *** ** * * ** ****** * * * ** *** ** * * * * * * ******* * * *** * ** * **** ***
 * TC47-TC53: reference evapotranspiration ETo (eq. 6) & ETc (eq. 56)
 *
 * TC47-TC50 use a composite scenario from verified FAO56 examples:
 *   delta = 0.1447 kPa/°C (TC1, T = 20°C);   Rn    = 7.6 MJ m-2 day-1 (TC29);
 *   gamma = 0.0674 kPa/°C (TC36);            Tmean = 20.0°C;
 *   es    = 2.3383 kPa (TC1);                ea    = 1.70 kPa (TC40), u2 = 2.0 m/s;
 *
 * TC47: G = 0 (daily), u2 = 2.0 -> ETo = 2.764 mm/day.
 ...
 *
 * TC48: G  =  1.0 (non-zero soil heat flux)       -> ETo = 2.535 mm/day
 * TC49: u2 =  0.0 (calm)   -> only radiation term -> ETo = 2.115 mm/day
 * TC50: Rn = -5.0 (winter) -> ETo < 0             -> clamped to 0.0 mm/day
 * TC51: STATUS_NULL_POINTER & STATUS_INVALID_VALUE for Calc_ETo
 * TC52: ETc = Kc * ETo = 1.15 * 2.764 = 3.179 mm/day
 * TC53: STATUS_NULL_POINTER & STATUS_INVALID_VALUE for Calc_ETc
 * * ** * * *** ** * * ** ****** * * * ** *** ** * * * * * * ******* * * *** * ** * **** * */

/* Common input data for TC47-TC50 ** * * ** ****** * * * ** *** ** * * * * * * ****** * * */
#define TEST_ETO_DELTA_KPA_C         (0.1447)   /* kPa/°C, from TC1 ** ** * * ** **** ** * */
#define TEST_ETO_GAMMA_KPA_C         (0.0674)   /* kPa/°C, from TC36 ** ** * * ** **** *** */
...
#define TEST_ETO_EA_KPA              (1.70)     /* kPa, from TC40 ** ** * * ** *** **** ** */
